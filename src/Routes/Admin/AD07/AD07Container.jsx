...
export default () => {
  ...
  const registerFaqTypeHandler = async () => {
    ...
    if (data.createFaqType) {
      toast.info("CREATE NEW FAQ TYPE!");
      faqTypeRefetch();
      setNewValue("");
    } else {
      toast.error("유형명을 기입 해 주시길 바랍니다.");
    }
  };
  ...
  const createFaqHandler = async () => {
    ...
    if (data.createFaq) {
      ...
      toast.error("질문과 답변을 작성해 주시길 바랍니다.");
    }
  };
